...
# ── Configuration ────────────────────────────────────────────────────────────
RAPIDAPI_KEY  = "0045d3b135msh9e5718195cf3506p139a1bjsnf6ec708460e7"
...
def _post(endpoint: str, payload: dict) -> Optional[dict]:
    """POST to RapidAPI endpoint and return parsed JSON or None on error."""
    try:
...
def _get_listing_urls(page_url: str) -> list[str]:
...
def fetch_coimbatore_houses(count: int = TARGET_COUNT) -> list[dict]:
    """
    Fetch `count` Coimbatore rental properties from Housing.com via RapidAPI.
    Returns a list of normalised property dicts.
    """
    all_urls: list[str] = []

    # Collect enough URLs across pages
    for page_url in COIMBATORE_RENTAL_PAGES:
        if len(all_urls) >= count * 2:   # fetch extra so we can filter
            break
        urls = _get_listing_urls(page_url)
    ...
    print(f"\n🔍 Fetching details for up to {count} properties …")
    ...
    # If API returned fewer than requested, pad with fallback data
    if len(houses) < count:
