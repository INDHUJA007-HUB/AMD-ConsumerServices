...
if os.path.exists(pg_file):
    ...
    with open(pg_file, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            try:
                price_str = str(row.get('price_per_month', '5000'))
                if 'Call' in price_str or not price_str.strip():
                    price = 5000
                else:
...
if os.path.exists(house_file):
    ...
    with open(house_file, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            try:
                price_str = str(row.get('price_per_month', '10000'))
                if 'Call' in price_str or not price_str.strip():
                    price = 10000
                else:
                    price = float(price_str.replace(',', '').replace('₹', '').strip())
