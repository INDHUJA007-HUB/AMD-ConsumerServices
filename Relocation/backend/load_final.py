import sys
import os
import csv
import json
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))

from database import SessionLocal, engine
import models

models.Base.metadata.create_all(bind=engine)
db = SessionLocal()

print("\n" + "="*60)
print("LOADING ALL DATASETS")
print("="*60)

db.query(models.Accommodation).delete()
db.query(models.Amenity).delete()
db.commit()
print("\n✓ Cleared existing data")

# Load PG data
pg_file = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'coimbatore_pg_dataset.csv')
pg_count = 0
if os.path.exists(pg_file):
    print(f"\nLoading PG data...")
    with open(pg_file, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
                accommodation = models.Accommodation(
                    name=row['pg_name'],
                    type='PG',
                    rent_price=5000,
                    latitude=float(row['latitude']),
                    longitude=float(row['longitude']),
                    safety_score=float(row.get('rating', 4.0)),
                    amenities=f"WiFi: {row.get('wifi', 'No')}, AC: {row.get('ac', 'No')}"
                )
                db.add(accommodation)
                pg_count += 1
            except:
                pass
    db.commit()
    print(f"✓ Loaded {pg_count} PGs")

# Load House data
house_file = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'coimbatore_houseonrent_dataset.csv')
house_count = 0
if os.path.exists(house_file):
    print(f"Loading House data...")
    with open(house_file, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
                accommodation = models.Accommodation(
                    name=row.get('pg_name', 'House'),
                    type='House',
                    rent_price=10000,
                    latitude=float(row['latitude']),
                    longitude=float(row['longitude']),
                    safety_score=float(row.get('rating', 4.0)) if row.get('rating') else 4.0,
                    amenities=row.get('room_type', 'N/A')
                )
                db.add(accommodation)
                house_count += 1
            except:
                pass
    db.commit()
    print(f"✓ Loaded {house_count} Houses")

# Load Restaurants
rest_file = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'coimbatore_zomato_restaurants.csv')
rest_count = 0
if os.path.exists(rest_file):
    print(f"Loading Restaurants...")
    with open(rest_file, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
                if row.get('latitude') and row.get('longitude'):
                    amenity = models.Amenity(
                        name=row.get('name', 'Restaurant'),
                        type='Restaurant',
                        latitude=float(row['latitude']),
                        longitude=float(row['longitude'])
                    )
                    db.add(amenity)
                    rest_count += 1
            except:
                pass
    db.commit()
    print(f"✓ Loaded {rest_count} Restaurants")

# Load coordinates as locations
coord_file = os.path.join(os.path.dirname(__file__), '..', 'datasets', 'coimbatore_2k_final.csv')
coord_count = 0
if os.path.exists(coord_file):
    print(f"Loading Location coordinates...")
    with open(coord_file, 'r', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        for i, row in enumerate(reader):
            try:
                if i % 100 == 0:  # Load every 100th point to avoid too much data
                    amenity = models.Amenity(
                        name=f"Location {i}",
                        type='Location',
                        latitude=float(row['latitude']),
                        longitude=float(row['longitude'])
                    )
                    db.add(amenity)
                    coord_count += 1
            except:
                pass
    db.commit()
    print(f"✓ Loaded {coord_count} Location points")

print("\n" + "="*60)
print("SUMMARY")
print("="*60)
print(f"PGs: {pg_count}")
print(f"Houses: {house_count}")
print(f"Restaurants: {rest_count}")
print(f"Locations: {coord_count}")
print(f"Total: {pg_count + house_count + rest_count + coord_count}")
print("="*60 + "\n")

db.close()
