...
const DashboardUserCard = () => {
  ...
  const refreshData = () => {
    if (typeof window !== "undefined") {
      ...
      setWorkplace(localStorage.getItem("workplace") || null);
      setStartingLocation(localStorage.getItem("startingLocation") || null);
      setPurpose(localStorage.getItem("purpose") || null);
    }
  };
  ...
  useEffect(() => {
    ...
    // Listen for custom storage updates from other components
    window.addEventListener("storage_update", refreshData);
    // Also listen for standard storage events (across tabs)
