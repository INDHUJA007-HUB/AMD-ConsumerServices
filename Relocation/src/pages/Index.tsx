...
const features = [
  { icon: <Brain className="h-6 w-6" />, title: "AI-Powered Decisions", desc: "Every suggestion backed by reasoning" },
...
const Index = () => {
  ...
  const handleSubmit = (_input: UserInput) => {
    // In future, this will call AI API. For now, use mock data.
    setPlan({ ...mockCityPlan, city: _input.city || mockCityPlan.city });
  };
    ...
    <AnimatePresence mode="wait">
      {plan ? (
        <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <Dashboard plan={plan} onBack={() => setPlan(null)} />
        </motion.div>
      ) : (
