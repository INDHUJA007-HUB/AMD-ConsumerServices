...
import { MapPin, Phone, Star, ExternalLink, Wifi, Snowflake, UtensilsCrossed, BedDouble } from "lucide-react";
...
import { loadPGs, PGItem } from "@/services/datasetsApi";
...
function getPrimaryImageUrl(item: PGItem) {
  const images = item.images ?? [];
  return images[0] ?? "";
}

function getStableId(item: PGItem, index: number) {
  return item.listingUrl || `${item.name ?? "pg"}-${item.area ?? ""}-${index}`;
}

const PGs = () => {
  const [query, setQuery] = useState("");
  const [pgsData, setPgsData] = useState<PGItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<PGItem | null>(null);
  const [carouselApi, setCarouselApi] = useState<CarouselApi | null>(null);

  useEffect(() => {
    async function fetchData() {
      const data = await loadPGs();
      setPgsData(data);
      setLoading(false);
    }
    fetchData();
  ...
  return (
    <Card className="border-blue-200 bg-white/80 backdrop-blur-sm">
      <CardHeader>
        ...
        <CardDescription>Browse PG stays. Click a card to view full details and photo gallery.</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((item, idx) => {
            ...
            const hasFood = normalizeYesNo(item.foodIncluded);
            const hasWifi = normalizeYesNo(item.wifi);
            const hasAc = normalizeYesNo(item.ac);
            ...
            return (
              ...
              >
                <div className="relative h-40 w-full bg-gradient-to-br from-blue-50 to-purple-50">
                  {primaryImage ? (
                    <img src={primaryImage} alt={item.name ?? "PG"} className="h-full w-full object-cover" loading="lazy" />
                  ) : (
                  ...
                  <div className="absolute top-3 left-3 flex gap-1.5">
                    ...
                    {item.pricePerMonth && (
                      <Badge className="bg-white/90 text-purple-700 border border-purple-100">
                        {item.pricePerMonth}
                      </Badge>
                    )}
                  </div>
                ...
                <div className="p-4 space-y-2">
                  <div className="min-w-0">
                    <p className="font-semibold text-sm text-gray-900 truncate">{item.name ?? "Unnamed PG"}</p>
            ...
            {selected && (
              <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
                <div className="space-y-3">
                  <DialogHeader>
                    <DialogTitle className="text-xl">{selected.name ?? "PG details"}</DialogTitle>
                        ...
                        {(selected.images ?? []).map((url, idx) => (
                          <CarouselItem key={`${url}-${idx}`} className="pl-0">
                            <div className="h-64 sm:h-80 w-full">
                              <img src={url} alt={`PG photo ${idx + 1}`} className="h-full w-full object-cover" loading="lazy" />
                            </div>
                          </CarouselItem>
                        ))}
                ...
                <div className="rounded-xl border border-blue-100 bg-white p-4 space-y-3">
                  ...
                  <div className="grid gap-2 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="font-medium">{selected.foodIncluded ?? "NA"}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-muted-foreground">Wi‑Fi</span>
                      <span className="font-medium">{selected.wifi ?? "NA"}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-muted-foreground">AC</span>
                      <span className="font-medium">{selected.ac ?? "NA"}</span>
                    </div>
                    <div className="flex items-center justify-between gap-3">
