(function(global,factory){if(typeof define==="function"&&define.amd){define("amazonLocationAuthHelper",["exports"],factory)}else if(typeof exports!=="undefined"){factory(exports)}else{var mod={exports:{}};factory(mod.exports);global.amazonLocationAuthHelper=mod.exports}})(typeof globalThis!=="undefined"?globalThis:typeof self!=="undefined"?self:this,function(_exports){"use strict";Object.defineProperty(_exports,"__esModule",{value:true});_exports.withAPIKey=withAPIKey;_exports.withCredentialProvider=withCredentialProvider;_exports.withIdentityPoolId=withIdentityPoolId;// Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
 ...
 */const presignUrl=(_ref13,_a)=>{let{body,method="GET",url}=_ref13;var options=__rest(_a,[]);const signingValues=getSigningValues(options);const{accessKeyId,credentialScope,longDate,sessionToken}=signingValues;// create the request to sign
const presignedUrl=new URL(url);Object.entries(Object.assign({[ALGORITHM_QUERY_PARAM$1]:SHA256_ALGORITHM_IDENTIFIER,[CREDENTIAL_QUERY_PARAM$1]:`${accessKeyId}/${credentialScope}`,[AMZ_DATE_QUERY_PARAM$1]:longDate,[SIGNED_HEADERS_QUERY_PARAM$1]:HOST_HEADER},sessionToken&&{[TOKEN_QUERY_PARAM$1]:sessionToken})).forEach(_ref14=>{let[key,value]=_ref14;presignedUrl.searchParams.append(key,value)});const requestToSign={body,headers:{[HOST_HEADER]:url.host},method,url:presignedUrl};// calculate and add the signature to the url
 ...
 * @internal
 */const getSigningValues=_ref15=>{let{credentials,signingDate=new Date,signingRegion,signingService,uriEscapePath=true}=_ref15;// get properties from credentials
 ...
 * @internal
 */const getSignature=(request/*: HttpRequest*/,_ref16)=>{let{credentialScope,longDate,secretAccessKey,shortDate,signingRegion,signingService,uriEscapePath}=_ref16;// step 1: create a canonical request
 ...
 * @internal
 */const getCanonicalQueryString=searchParams=>Array.from(searchParams).sort((_ref18,_ref19)=>{let[keyA,valA]=_ref18;let[keyB,valB]=_ref19;if(keyA===keyB){return valA<valB?-1:1}return keyA<keyB?-1:1}).map(_ref20=>{let[key,val]=_ref20;return`${escapeUri(key)}=${escapeUri(val)}`}).join("&");const escapeUri=uri=>encodeURIComponent(uri).replace(/[!'()*]/g,hexEncode);const hexEncode=c=>`%${c.charCodeAt(0).toString(16).toUpperCase()}`;/**
 ...
 * @internal
 */const getCanonicalHeaders$1=headers=>Object.entries(headers).map(_ref21=>{let[key,value]=_ref21;var _a;return{key:key.toLowerCase(),value:(_a=value===null||value===void 0?void 0:value.trim().replace(/\s+/g," "))!==null&&_a!==void 0?_a:""}}).sort((a,b)=>a.key<b.key?-1:1).map(entry=>`${entry.key}:${entry.value}\n`).join("");/**
 ...
 */function createTransformRequest(region,getCredentials){return(url,resourceType)=>{// Only sign Amazon Location Service URLs
...
serviceName="geo"}const credentials=getCredentials();return{url:Signer.signUrl(url,region,serviceName,{access_key:credentials.accessKeyId,secret_key:credentials.secretAccessKey,session_token:credentials.sessionToken})}}return{url}}}/**
 ...
 */async function setupCredentialRefresh(credentialProvider){let credentials;async function refreshCredentials(){credentials=await credentialProvider();let timeToRefresh=3600000;// default to 1 hour if credentials does not have expiration field
...
setTimeout(refreshCredentials,timeToRefresh-60000)}await refreshCredentials();return()=>credentials}/**
 ...
 */async function buildAuthHelper(credentialProvider,region){const getCredentials=await setupCredentialRefresh(credentialProvider);const clientConfig={credentials:credentialProvider,region};return{getMapAuthenticationOptions:()=>({transformRequest:createTransformRequest(region,getCredentials)}),getLocationClientConfig:()=>clientConfig,getClientConfig:()=>clientConfig,getCredentials}}// Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0
/**
 * Creates an auth helper instance using credentials from Cognito.
 *
 * @param identityPoolId Cognito Identity Pool Id
 */async function withIdentityPoolId(identityPoolId,options){const region=identityPoolId.split(":")[0];const credentialProvider=fromCognitoIdentityPool(Object.assign(Object.assign({},options||{}),{identityPoolId,clientConfig:Object.assign(Object.assign({},options&&options.clientConfig?options.clientConfig:{}),{region})}));return buildAuthHelper(credentialProvider,region)}// Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0
/**
...
if(region){clientConfig.region=region}return{getLocationClientConfig:()=>clientConfig,getClientConfig:()=>clientConfig}}// Copyright Amazon.com, Inc. or its affiliates. All Rights Reserved.
// SPDX-License-Identifier: Apache-2.0
/**
